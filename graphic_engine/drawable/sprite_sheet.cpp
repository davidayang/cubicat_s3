...
#include <cmath>
SpriteSheet::SpriteSheet(uint16_t width, uint16_t height,const uint16_t* data, uint16_t col, uint16_t row, bool hasMask, uint16_t maskColor,
...
void SpriteSheet::setFrame(int nth) {
    ...
    m_frameDataPtr = m_data + m_currentFrame*w*h / (16 / m_bpp);
    m_maskColor = m_originMaskColor;
}
void SpriteSheet::setFrameData(const uint16_t* data, uint16_t width, uint16_t height, uint16_t maskColor) {
