...
int BBCapTouch::I2CReadRegister16(uint8_t u8Addr, uint16_t u16Register, uint8_t *pData, int iLen)
{
    ...
    ESP_ERROR_CHECK(i2c_master_write_read_device(I2C_NUM_0, u8Addr, ucTemp, 2, pData, iLen, 100));
    i = iLen;
#endif
    return i;
...
//      Serial.printf("I2CReadRegister returned %d\n", i);
    ...
    if (_iType == CT_TYPE_CST820)
    {
        ...
        if (ucTemp[0] < 1 || ucTemp[0] > 5)
        { // something went wrong
            return 0;
        }
        if (ucTemp[0] >= 1)
        { // touch data available, read it
            pTI->count = ucTemp[0];
    ...
    if (_iType == CT_TYPE_FT6X36)
    {
        ...
        {                                                              // get data
            ...
            if ((ucTemp[0] & 0x40) == 0 && (ucTemp[2] & 0xf0) != 0xf0)
            { // finger is down
            ...
            if (i > 1)
            { // get second point
                if ((ucTemp[6] & 0x40) == 0 && (ucTemp[8] & 0xf0) != 0xf0)
                { // finger is down
    ...
    {                                                           // GT911
        ...
        if (i <= 5 && ucTemp[0] & 0x80)
        { // if buffer status is good + >= 1 touch points
