...
#include "core/memory_allocator.h"
    ...
    #include "esp_log.h"
    #define log_e(...) ESP_LOGE("mp3_decoder", __VA_ARGS__)
    #define log_i(...) ESP_LOGI("mp3_decoder", __VA_ARGS__)
...
bool FLACDecoder_AllocateBuffers(void){
    if(psram_init()) {
        ...
        if(!FLACFrameHeader)    {FLACFrameHeader   = (FLACFrameHeader_t*)    psram_malloc(sizeof(FLACFrameHeader_t));}
        if(!FLACMetadataBlock)  {FLACMetadataBlock = (FLACMetadataBlock_t*)  psram_malloc(sizeof(FLACMetadataBlock_t));}
        if(!FLACsubFramesBuff)  {FLACsubFramesBuff = (FLACsubFramesBuff_t*)  psram_malloc(sizeof(FLACsubFramesBuff_t));}
    }
    else {
