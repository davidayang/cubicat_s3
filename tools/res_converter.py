...
def createImageCpp(img_name, col, row, has_trans, palette):
    ...
    height = math.floor(image.size[1] / row)
    first_color = None
    packedData = 0x0000
    packCounter = 0
    for r in range(row):
        for c in range(col):
            for h in range(height):
                for w in range(width):
                    ...
                    if palette:
                        val = 1 if (R > 8 and G > 16 and B > 8) else 0
                        if val > 0:
                            packedData |= val << (15 - packCounter)
                        packCounter += 1
                        if packCounter >= 16 or end:
                            out_data.append(rgb_to_hex(packedData))
                            packCounter = 0
                            packedData = 0x0000
                    else:
                        val = rgb_to_hex(rgb)
                        out_data.append(val)
    content = ','.join(out_data)
    ...
    with open("generated/imgs/" + filename + ".cpp", 'w') as file:
        ...
        file.write(f"ImageData {filename} = {{\n")
...
def getSoundData(wav_file):
    ...
    wav_file.close()
    # # 计算文件头的字节数
    # header_size = 44  # 通常WAV文件的文件头大小为44字节
    # # 计算去除文件头后的音频数据字节数
    # data_size = len(audio_data) - header_size
    # # 去除文件头，仅保留原始音频数据
    # audio_data = audio_data[header_size:]
    return audio_data


def writeBMFontHeader(file, font_size, chars):
    # 写入字号
    file.write(struct.pack('I', font_size))
    char_cnt = len(chars)
    # 写入字符集个数
    file.write(struct.pack('I', char_cnt))
    # 写入数据偏移
    utf_bytes = chars.encode(UTF_CODE)
    byte_length = len(utf_bytes)
    file.write(struct.pack('I', 4 + 4 + 4 + byte_length))
    # 写入字符集
    file.write(utf_bytes)


# 文件头格式:
# 字体大小(4byte)|字数(4byte)|字模数据偏移(4byte)|字库数据...|字模数据...
def readBMFontHeader(file):
    font_size = int.from_bytes(file.read(4), byteorder='little')
    char_count = int.from_bytes(file.read(4), byteorder='little')
    data_offset = int.from_bytes(file.read(4), byteorder='little')
    char_set = file.read(data_offset - 12)
    return font_size, char_count, data_offset, char_set


def generateBitmapFont(font_file, font_size):
    if font_size > 48:
        font_size = 48
    # 加载字体
    font = ImageFont.truetype(font_file, font_size)
    preFix = "GB2312_"
    with open(f"generated/temp/gb2313font_{font_size}.fnt", "wb") as f:
        ...
        FONT_ASC_RAW = r'!\"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~'
        FONT_ALL = FONT_ASC + FONT_GB3212
        writeBMFontHeader(f, font_size, FONT_ALL)
        # 创建一个空白图像，最大尺寸 64
        img_w = 64
        img_h = 64
        image = Image.new('P', (img_w, img_h), color='white')
        # 创建一个 Draw 对象
        draw = ImageDraw.Draw(image)
        # 渲染文本
        text_color = (0, 0, 0)  # 黑色
        # 16号字体每个字符数据有 36字节（4byte的bbox数据 + 16x16=256bit=32byte的字符点阵数据，1个点一个bit）
        for char in FONT_ALL:
            bbox = list(font.getbbox(char))  # 返回 (x0, y0, x1, y1)
            if bbox[0] < 0:
                bbox[0] = 0
            if bbox[1] < 0:
                bbox[1] = 0
            # 保存bbox
            f.write(bytes([bbox[0], bbox[1], bbox[2], bbox[3]]))
            draw.rectangle([0, 0, img_w, img_h], fill=(255, 255, 255))  # 白色填充
            draw.text((0, 0), char, fill=text_color, font=font)
            c = 0
            # font_size x font_size dot
            dot_data = bytearray(math.ceil(font_size * font_size / 8))
            for y in range(bbox[1], bbox[3]):
                for x in range(bbox[0], bbox[2]):
                    if c < font_size * font_size:
                        index = math.floor(c / 8)
                        bit_offset = c % 8
                        c += 1
                        p = image.getpixel((x, y))
                        if p > 0:
                            dot_data[index] |= 0x1 << (7 - bit_offset)
            f.write(dot_data)
    # 同时数据保存为.h文件和.cpp以供嵌入式使用
    with open(f"generated/temp/gb2313font_{font_size}.fnt", "rb") as font_file:
        with open(f"generated/fonts/gb2312font_{font_size}.h", "w") as hfile:
            hfile.write("// Automatic generated, DO NOT EDIT!\n")
            hfile.write("# pragma once\n")
            hfile.write("#include <cstdint>\n")
            hfile.write(f"extern const uint8_t {preFix}GlyphData_{font_size}[];\n")
            hfile.write(f"extern const uint8_t {preFix}FontSize_{font_size};\n")
            hfile.write(f"extern const uint16_t {preFix}CharCount_{font_size};\n")
            hfile.write(f"extern const char* {preFix}CharSet_{font_size};\n")
        with open(f"generated/fonts/gb2312font_{font_size}.cpp", "w") as cppfile:
            # 读取font文件头
            _font_size, char_count, data_offset, char_set = readBMFontHeader(font_file)
            cppfile.write("// Automatic generated, DO NOT EDIT!\n")
            cppfile.write(f'#include "graphic_engine/drawable/gb2312font_{font_size}.h"\n')
            cppfile.write('#ifdef ARDUINO\n')
            cppfile.write('#include <pgmspace.h>\n')
            cppfile.write('#else\n')
            cppfile.write('#ifndef PROGMEM\n')
            cppfile.write('#define PROGMEM\n')
            cppfile.write('#endif\n')
            cppfile.write('#endif\n')
            cppfile.write(f"const uint8_t {preFix}GlyphData_{font_size}[] PROGMEM = {{ \n")
            glyph_data = font_file.read()
            cppfile.write(",".join(str(byte) for byte in glyph_data))
            cppfile.write("};\n")
            cppfile.write(f"const uint8_t {preFix}FontSize_{font_size} = {_font_size};\n")
            cppfile.write(f"const uint16_t {preFix}CharCount_{font_size} = {char_count};\n")
            raw_char_set = FONT_ASC_RAW + FONT_GB3212
            cppfile.write(f'const char* {preFix}CharSet_{font_size} = "{raw_char_set}";\n')


def vector_angle(v):
    # 计算与 x 轴的夹角（以弧度为单位）
    angle_radians = math.atan2(v[1], v[0])
...
def vector_sub(v0, v1):
    return [v0[0] - v1[0], v0[1] - v1[1]]


def generateMountingPoint(img, col, row, markerColor):
    image = Image.open(img)
    if image.mode != 'RGB':
        image = image.convert('RGB')
    pixel_data = image.load()
    frame_width = math.floor(image.size[0] / col)
    frame_height = math.floor(image.size[1] / row)
    points = []
    for r in range(row):
        for c in range(col):
            has_mpt = False
            for h in range(frame_height):
                for w in range(frame_width):
                    ...
                    if color[0] << 16 | color[1] << 8 | color[2] == markerColor:
                        points.append([w, frame_height - h])
                        has_mpt = True
            if not has_mpt:
                points.append([999, 999])
                points.append([999, 999])
    filename = img.split('.')[0]
    with open("generated/mounting_point/" + filename + ".cpp", 'w') as cppFile:
        cppFile.write("#include <pgmspace.h>\n")
        cppFile.write("#include <cstdint>\n")
        cppFile.write('#include "mounting_points.h"\n')
        cppFile.write(f"const int16_t {filename.upper()}[] PROGMEM = {{\n")
        for i in range(0, len(points), 2):
            pt0 = points[i]
            pt1 = points[i + 1]
            if pt0[1] < pt1[1]:
                _dir = vector_sub(pt1, pt0)
                cppFile.write(f"{pt0[0]},{pt0[1]},{vector_angle(_dir) - 90},")
            else:
                _dir = vector_sub(pt0, pt1)
                cppFile.write(f"{pt1[0]},{pt1[1]},{vector_angle(_dir) - 90},")
        cppFile.write("\n};\n")
        cppFile.write(f"MountingPoint {filename} = {{\n")
        cppFile.write(f"    .len = {len(points)},\n")
        cppFile.write(f"    .data = {filename.upper()}\n")
        cppFile.write("};\n")
...
if __name__ == '__main__':
    ...
    res_path = ""
    if len(res_path) == 0:
        print("resource path not specified!!")

    if os.path.exists("generated"):
        shutil.rmtree("generated")
    os.makedirs("generated", exist_ok=True)
    os.makedirs("generated/imgs", exist_ok=True)
    os.makedirs("generated/sounds", exist_ok=True)
    os.makedirs("generated/fonts", exist_ok=True)
    os.makedirs("generated/mounting_point", exist_ok=True)
    ...
    with open("generated/imgs/images.cpp", 'w') as img_cpp_file:
        img_cpp_file.write('#include <map> \n')
        img_cpp_file.write('#include <string> \n')
        img_cpp_file.write('#include "images.h"\n')
        img_cpp_file.write('std::map<std::string, ImageData> g_imgMap = {\n')
        with open("generated/imgs/images.h", 'w') as file:
            file.write('// Automatic generated, DO NOT EDIT!\n')
            file.write("#pragma once\n")
            file.write("#ifdef ARDUINO\n")
            file.write("#include <pgmspace.h>\n")
            file.write("#else\n")
            file.write('#ifndef PROGMEM\n')
            file.write("#define PROGMEM\n")
            file.write('#endif\n')
            file.write("#endif\n")
            file.write("#include <cstdint>\n")
            file.write('#include "graphic_engine/drawable/image_data.h"\n')
            file.write('extern const ImageData& getImageDataByResName(const char* name);\n')
            for img, dim in imgs.items():
                obj = img.split(".")[0] + "_img"
                file.write(f"extern ImageData {obj};\n")
                col = dim[0]
                row = dim[1]
                trans = dim[2]
                palette = dim[3]
                createImageCpp(res_path + "/" + img, col, row, trans, palette)
                img_cpp_file.write(f'   {{"{obj}", {obj} }},\n')
        img_cpp_file.write(' };\n')
        img_cpp_file.write('const ImageData& getImageDataByResName(const char* name) {\n')
        img_cpp_file.write('    auto itr = g_imgMap.find(name);\n')
        img_cpp_file.write('    assert(itr != g_imgMap.end());\n')
        img_cpp_file.write('    return itr->second;\n')
        img_cpp_file.write(' };\n')
    # convert sounds files
    if len(sounds) > 0:
        with open("generated/sounds/sounds.h", 'w') as file:
            file.write("#pragma once\n")
            for sound in sounds:
                objName = sound.split(".")[0] + "_sound"
                file.write(f"extern const uint8_t {objName.upper()}[];\n")
        with open("generated/sounds/sounds.cpp", 'w') as file:
            for sound in sounds:
                objName = sound.split(".")[0] + "_sound"
                content = getSoundData(sound)
                file.write(f"const uint8_t {objName.upper()}[] PROGMEM = {{ \n")
                file.write(",".join(str(byte) for byte in content))
                file.write("};\n")
    # convert fonts
    for font, s in fonts.items():
        generateBitmapFont(res_path + "/" + font, s)
    # generate mounting point
    if len(mountingPoints) > 0:
        with open("generated/mounting_point/mounting_points.cpp", 'w') as mpt_cpp_file:
            mpt_cpp_file.write('#include <map> \n')
            mpt_cpp_file.write('#include "mounting_points.h"\n')
            mpt_cpp_file.write('std::map<std::string, MountingPoint> g_mountingPointMap = {\n')
            with open("generated/mounting_point/mounting_points.h", "w") as file:
                file.write("#pragma once\n")
                file.write("#include <stdio.h>\n")
                file.write('#include "graphic_engine/mounting_point.h"\n')
                file.write("extern MountingPoint getMountingPointDataByResName(const char* name);\n")
                for img, dim in mountingPoints.items():
                    file.write(f"extern MountingPoint {img.split('.')[0]};\n")
                    generateMountingPoint(img, dim[0], dim[1], dim[2])
                    filename = img.split(".")[0]
                    mpt_cpp_file.write(f'   {{"{filename}", {filename} }},\n')
            mpt_cpp_file.write("};\n")
            mpt_cpp_file.write('MountingPoint getMountingPointDataByResName(const char* name) {\n')
            mpt_cpp_file.write('    auto itr = g_mountingPointMap.find(name);\n')
            mpt_cpp_file.write('    assert(itr != g_mountingPointMap.end());\n')
            mpt_cpp_file.write('    return itr->second;\n')
            mpt_cpp_file.write(' };\n')
