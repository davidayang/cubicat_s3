...
typedef ffi_word_t (*wwfff_t)(ffi_word_t, float, float, float);
typedef ffi_word_t (*wwwwf_t)(ffi_word_t, ffi_word_t, ffi_word_t, float);
typedef ffi_word_t (*wffff_t)(float, float, float, float);
...
#define FUNC_MISS \
    if (nargs == 3) { \
        const char* p0 = IS_F(args[0]) ? "f" : (IS_D(args[0]) ? "d" : "w"); \
        const char* p1 = IS_F(args[1]) ? "f" : (IS_D(args[1]) ? "d" : "w"); \
        const char* p2 = IS_F(args[2]) ? "f" : (IS_D(args[2]) ? "d" : "w"); \
        printf("function missing: %s %s %s res->ctype %d at line: %d\n", p0, p1, p2,res->ctype, __LINE__); \
    } else if (nargs == 4) { \
        const char* p0 = IS_F(args[0]) ? "f" : (IS_D(args[0]) ? "d" : "w"); \
        const char* p1 = IS_F(args[1]) ? "f" : (IS_D(args[1]) ? "d" : "w"); \
        const char* p2 = IS_F(args[2]) ? "f" : (IS_D(args[2]) ? "d" : "w"); \
        const char* p3 = IS_F(args[3]) ? "f" : (IS_D(args[3]) ? "d" : "w"); \
        printf("function missing: %s %s %s %s res->ctype %d at line: %d\n", p0, p1, p2,p3,res->ctype, __LINE__); \
    } else if (nargs == 5) { \
        const char* p0 = IS_F(args[0]) ? "f" : (IS_D(args[0]) ? "d" : "w"); \
        const char* p1 = IS_F(args[1]) ? "f" : (IS_D(args[1]) ? "d" : "w"); \
        const char* p2 = IS_F(args[2]) ? "f" : (IS_D(args[2]) ? "d" : "w"); \
        ...
        printf("function missing: %s %s %s %s %s res->ctype %d at line: %d\n", p0, p1, p2,p3,p4,res->ctype, __LINE__); \
    } else { \
        const char* p0 = IS_F(args[0]) ? "f" : (IS_D(args[0]) ? "d" : "w"); \
        const char* p1 = IS_F(args[1]) ? "f" : (IS_D(args[1]) ? "d" : "w"); \
        const char* p2 = IS_F(args[2]) ? "f" : (IS_D(args[2]) ? "d" : "w"); \
        ...
        if (floats == 0) {
          ...
          switch (nargs) {
            ...
            case 3:
              ...
              break;
            case 4:
                if (IS_W(args[0]) && IS_F(args[1]) && IS_F(args[2]) && IS_F(args[3])) {
                    wwfff_t f = (wwfff_t) func;
                    r = f (W(args[0]), F(args[1]), F(args[2]), F(args[3]));
                } else if (IS_W(args[0]) && IS_W(args[1]) && IS_W(args[2]) && IS_F(args[3])) {
                    wwwwf_t f = (wwwwf_t) func;
                    r = f (W(args[0]), W(args[1]), W(args[2]), F(args[3]));
                } else if (IS_F(args[0]) && IS_F(args[1]) && IS_F(args[2]) && IS_F(args[3])) {
                    wffff_t f = (wffff_t) func;
                    r = f (F(args[0]), F(args[1]), F(args[2]), F(args[3]));
                } else {
                    FUNC_MISS
                    abort();
                }
                break;
            case 5:
                if (IS_W(args[0]) && IS_F(args[1]) && IS_F(args[2]) && IS_F(args[3]) && IS_F(args[4])) {
                    wwffff_t f = (wwffff_t) func;
                    r = f (W(args[0]), F(args[1]), F(args[2]), F(args[3]), F(args[4]));
                } else if (IS_W(args[0]) && IS_W(args[1]) && IS_F(args[2]) && IS_F(args[3]) && IS_F(args[4])) {
                    wwwfff_t f = (wwwfff_t) func;
                    r = f (W(args[0]), W(args[1]), F(args[2]), F(args[3]), F(args[4]));
                } else if (IS_W(args[0]) && IS_W(args[1]) && IS_W(args[2]) && IS_F(args[3]) && IS_F(args[4])) {
                    wwwwff_t f = (wwwwff_t) func;
                    r = f (W(args[0]), W(args[1]), W(args[2]), F(args[3]), F(args[4]));
                ...
                break;
            case 6:
                if (IS_W(args[0]) && IS_W(args[1]) && IS_W(args[2]) && IS_F(args[3]) && IS_F(args[4]) && IS_W(args[5])) {
                    wwwwffw_t f = (wwwwffw_t) func;
                    r = f (W(args[0]), W(args[1]), W(args[2]), F(args[3]), F(args[4]), W(args[5]));
                } else {
                    FUNC_MISS
                    abort();
                }
...
static mjs_ffi_ctype_t parse_cval_type(struct mjs *mjs, const char *s,
  ...
  } else if (mg_vcmp(&ms, "unsigned int") == 0 || mg_vcmp(&ms, "uint32_t") == 0) {
    return MJS_FFI_CTYPE_INT;
...
MJS_PRIVATE mjs_err_t mjs_ffi_call2(struct mjs *mjs) {
  ...
  int result = ffi_call(psig->fn, nargs, &res, args);
  if (result < 0) {
        char param_buf[256] = {0};
        char* addr = param_buf;
        for (int i = 0; i < nargs; i++) {
            if (i == nargs - 1) {
                sprintf(addr, "%d", args[i].ctype);
                break;
            }
            sprintf(addr, "%d, ", args[i].ctype);
            addr = addr + 3;
        }
      printf("ffi_call failed: nargs %d (%s) return type %d\n", nargs, param_buf, res.ctype);
  }
...
static mjs_err_t parse_function(struct pstate *p) {
  ...
  if (p->tok.tok == TOK_IDENT) {
    ...
    emit_byte(p, OP_PUSH_STR);
    emit_str(p, tmp.ptr, tmp.len);
    sprintf(funcName, "%.*s", tmp.len, tmp.ptr);
